...
# physeq generation from qiime data
...
sample_data(physeq)$time.point <- as.factor(sample_data(physeq)$time.point)
...
# ancombc
out <- ancombc(phyloseq = ps.taxa.sub, tax_level = "Genus", formula = "body.site", 
               p_adj_method = "holm", prv_cut = 0.10, lib_cut = 1000, 
               group = "body.site", struc_zero = TRUE, neg_lb = TRUE, tol = 1e-5, 
               max_iter = 100, conserve = TRUE, alpha = 0.05, global = TRUE)
res <- out$res
# gan q (p hieu chinh) value vs different abundance 
q <- res[["q_val"]]
q<-q %>% rownames_to_column("taxon")
diff <- res[["diff_abn"]]
aa <- inner_join(q, diff, by = "taxon")
# loai bo family, order, class,NA
aa1 <- na.omit(aa)
aa1 <- aa[!grepl("Order:", aa$taxon),]
aa2 <- aa1[!grepl("Family:", aa1$taxon),]
aa3 <- aa2[!grepl("Class:", aa2$taxon),]
aa4 <- aa3[!grepl("Phylum:", aa3$taxon),]
aa5 <- aa4[!grepl("Kingdom:", aa4$taxon),]
# lay top 10
aa4 <- aa3%>%filter(intercept < 3e-12)
# xoa "Genus:"
top10 <- aa4 %>%  mutate(taxon = str_replace(string = taxon, 
                                             pattern = "Genus:", 
                                             replacement = ""))
tax$Species <- tax$Species %>% replace_na("Unclassified")
#chuan bi composite
#import data:
metadata<-read_q2metadata("sample-metadata.tsv")
...
freq <- read_tsv("significant differential/freq.tsv") %>%
  select(`sample ID`, starts_with("ASV")) %>%
  pivot_longer(-`sample ID`, names_to="ASVs", values_to="count")
...
composite <- inner_join(freq,taxonomy,by="ASVs") %>% 
  ...
  select (-count) %>% 
  inner_join(.,metadata, by = c("sample ID"="SampleID"))
guttongue <- composite%>%filter(`body-site` == "gut" | `body-site` == "tongue")
# xoa g__
guttongue1 <- guttongue %>% 
  mutate(genus = str_replace(string = genus, 
                             pattern = "g__", 
                             replacement = ""))
#Tinh gia tri trung binh abundance o trong tung nhom
mean_tax<-guttongue1%>%
  group_by(`body-site`,genus)%>%
  summarize(mean_abundance = mean(abundance),.groups="drop")
# ghep top 10 voi mean_tax
aa5 <- inner_join(top10,mean_tax, by = c("taxon" = "genus"))

# ve barplot
chuyen gtri mean_abundance gut ve -1
aa8%>%
  mutate(mean_abundance = if_else(`body-site` == "gut", -1*mean_abundance,mean_abundance), 
         taxon = fct_reorder(taxon, mean_abundance)) %>%
  ggplot(aes(x=mean_abundance, y = taxon, fill = `body-site`)) + 
  geom_col() + 
  labs(y = "Taxonomy", x = "Mean proportion(%)")+
...
aa7%>%ggplot(aes(y=taxon, x=TypeDA,fill=Type )) + 
  geom_col() +
  theme_minimal() + 
  labs(x="Log Fold Change", y=NULL) + 
  scale_fill_discrete(name=NULL,
                      breaks = c("BA","DA"),
                      labels = c("Bacimix Used","Diarrhea Control"))+
